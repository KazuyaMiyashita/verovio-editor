...
export class EditorCursorPointer {
  ...
  xToMEI(x: number): number {
    return (
      Math.round(x - this.viewLeft + this.scrollLeft) * this.pixPerPix -
      this.marginLeft
    );
  }
  ...
  yToMEI(y: number): number {
    return Math.round(
      (y - this.viewTop + this.scrollTop) * this.pixPerPix - this.marginTop,
    );
  }

  xToView(x: number): number {
    return (
      (x + this.marginLeft) / this.pixPerPix - this.scrollLeft + this.viewLeft
    );
  }

  yToView(y: number): number {
    return (
      (y + this.marginTop) / this.pixPerPix - this.scrollTop + this.viewTop
    );
  }
