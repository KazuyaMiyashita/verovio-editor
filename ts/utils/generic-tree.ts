...
export class GenericTree extends GenericView {
  ...
  public isInFocus(node: TreeNode) {
    if (!this.hasFocus() || this.id === this.focusId) return true;
  ...
  public isDescendantOfFocus(node: TreeNode) {
    const focusNode = this.findInSubtree(
      this.root,
      (node: TreeNode) => node.id === this.focusId,
    );
  ...
  public getDisplayDepth(): number {
    return this.displayDepth;
  }
  public getFocusId(): string {
    return this.focusId;
  }
  ...
  public addCrumb(element: string, id: string): void {
    const crumb: HTMLDivElement = appendDivTo(this.breadCrumbs, {
  ...
  protected collapseNode(id: string) {
    this.traverse((node) => {
      if (node.id === id) {
        ...
        const children = node
          .getDiv()
          .querySelector(".vrv-node-children") as HTMLElement;
  ...
  private buildTreeFromJson(nodeData: any): TreeNode {
    ...
    if (Array.isArray(children)) {
      node.setChildren(
        children.map((child: any) => this.buildTreeFromJson(child)),
      );
    }
    return node;
  }
  ...
  private buildTreeFromElement(element: Element): TreeNode {
    ...
    for (let node of Array.from(element.childNodes)) {
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (textContent) {
          children.push(
            new TreeNode(null, "#text", { textContent }, [], true, true),
          );
        }
      }
    }
...
export class TreeNode {
  ...
  public html(
    ...
    if (depth >= tree.getDisplayDepth() || !isInFocusSubtree) return;
