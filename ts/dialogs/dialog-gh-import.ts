...
export class DialogGhImport extends Dialog {
  ...
  public getData(): string | ArrayBuffer | Blob {
    return this.data;
  }
  ...
  protected updateSelectionAndBreadcrumbs(): void {
    ...
    const icon: string =
      this.githubManager.getSelectedOrganization() !== null
    ...
    if (
      !this.addSelection(
        this.githubManager.getSelectedRepoName(),
        this.iconsRepo,
      )
    )
      return;
    if (
      !this.addSelection(
        this.githubManager.getSelectedBranchName(),
        this.iconsBranch,
      )
    )
      return;
  ...
  private loadingStart(tab: HTMLDivElement): void {
    Array.from(this.tabs.querySelectorAll(".vrv-tab-selector")).forEach(
      (node) => {
  ...
  private addItemToList(
    ...
    const item: HTMLDivElement = appendDivTo(this.list, {
  ...
  private addSelection(name: string, icon: string): boolean {
    ...
    const selection: HTMLDivElement = appendDivTo(this.selection, {
  ...
  protected async selectFile(e: MouseEvent): Promise<any> {
    ...
    if (element.dataset.type === "dir") {
      ...
      const filename =
        this.githubManager.getPathString() + "/" + element.dataset.name;
  ...
  private async listUsers(): Promise<any> {
    ...
    const userChecked =
      this.githubManager.getSelectedAccountName() ===
      this.githubManager.getLogin();
    this.addItemToList(
      this.githubManager.getLogin(),
      this.iconsUser,
      { login: this.githubManager.getLogin() },
      userChecked,
      this.selectUser,
    );
