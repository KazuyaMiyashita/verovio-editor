...
export class ResponsiveView extends VerovioView {
  ...
  public async midiUpdate(time: number): Promise<any> {
    ...
    let vrvTime = time;
    let elementsAtTime = (await this.app.verovio.getElementsAtTime(
      vrvTime,
    ...
    if (
      ...
      if (elementsAtTime.measure) {
        ...
        elementsAtTime.page = await this.app.verovio.getPageWithElement(
          elementsAtTime.measure,
        );
      }
    }
    ...
    if (
      ...
      for (let i = 0, len = this.midiIds.length; i < len; i++) {
        let note = <SVGElement>(
          this.svgWrapper.querySelector("#" + this.midiIds[i])
        );
