...
export class DocumentView extends VerovioView {
  ...
  constructor(div: HTMLDivElement, app: App, verovio: VerovioWorkerProxy) {
    ...
    try {
      ...
      console.info(
        "IntersectionObserver support is missing - loading all pages",
      );
    }
  }
  ...
  override async refreshView(
    update: VerovioView.Refresh,
    lightEndLoading: boolean = true,
    mei: string = "",
  ...
  async updateLoadData(
    ...
    for (let idx = 0; idx < this.app.getPageCount(); idx++) {
      ...
      if (!this.app.options.documentViewSVG) {
        ...
        ctx.fillText(
          "Loading ...",
          this.currentPageWidth / 2,
          this.currentPageHeight / 6,
        );
    ...
    if (this.app.options.documentViewSVG) {
      ...
      for (let idx = 0; idx < this.app.getPageCount(); idx++) {
        ...
        if (page.firstChild && page.firstChild) {
          (<Element>page.firstChild).setAttribute(
            `height`,
            `${this.currentPageHeight}px`,
          );
          (<Element>page.firstChild).setAttribute(
            `width`,
            `${this.currentPageWidth}px`,
          );
        }
      }
    } else {
  ...
  private handleObserver(
    ...
    for (let entry of entries) {
      if (entry.isIntersecting) {
        ...
        observer.lastPageIn = parseInt(
          (<HTMLElement>entry.target).dataset.page,
        );
      }
    }
  }
  ...
  private async renderPage(pageIndex): Promise<any> {
    ...
    // SVG
    if (this.app.options.documentViewSVG) {
      const scaleSvg = this.parseAndScaleSVG(
        svg,
        this.currentPageHeight,
        this.currentPageWidth,
      );
